...
unsigned long opc, int size, Dyninst::Address firstAddr, dp::Block *xxb, bool findp){
    ...
    switch(opc){
        ...
        case 0x4b://call
            ...
            }else if(blabel[b].first == 0x1 || blabel[b].first == 0x4 || blabel[b].first == 0x6){
                odata tp[1];
                memset(tp, 0, sizeof(tp));
...
void findallPaths(dp::Function const *f,dp::Block *root, std::vector<dp::Block*> &path, dp::Block *xxb, bool findp) {
	...
	for (dp::Edge *e : root->targets()) {
        ...
        if (e->type() == dp::CALL){
			...
			if(func == 1 || func ==4 || func == 2 || func == 6){//adding new alloc method, need to change here
	...
	if(isret){
		...
		for (int i = 0; i < path.size() - 1; ++i) {
			...
			if(blabel[path[i]].first == 1 || blabel[path[i]].first == 4 || blabel[path[i]].first == 6){//only fit to single obj
				m = i;
...
int main(int argc, char *argv[]) {
	...
	for (auto const *f : all) {
		...
		std::cout << "\t subgraph cluster_" << cluster_index << " { \n\t\t label=\""
              << f->name() << "\"; \n\t\t color=blue;" << '\n';
		if(std::strcmp(f->name().c_str(), argv[fnum]) != 0){
						...
						if(eachlink[jk].first == 0){
							if(ubbs.size() == 0 && isfirst){
								...
								*/
								...
								for(dp::Block * pp : temps){
                                    cout << pp->start() << " ->";
                                }
                                cout << "\n";
								cout << "BBBBBBBBBBBBBBBBBBBBBBBBB\n";
								cout << "CCCCCCCCCCCCCCCCCCCCCCCCC\n";
								for(dp::Block * pp : temps2){
                                    cout << pp->start() << " ->";
                                }
                                cout << "\n";
								cout << "CCCCCCCCCCCCCCCCCCCCCCCCC\n";
		...
		postbb = tubbs;
		/*for(dp::Edge *e : tues){
			if(std::find(postbb.begin(), postbb.end(), e->trg()) == postbb.end()){
				postbb.push_back(e->trg());
			}
		}*/
		...
		for (dp::Block *pb : postbb) {
			...
			/*cout << "-----------------------------------------------\n";
			cout << "b->start(): "<< pb->start() << "\n";
			cout << "b->end(): "<< pb->end() << "\n";
			cout << "b->last(): "<< pb->last() << "\n";
			cout << "b->lastInsnAddr(): "<< pb->lastInsnAddr() << "\n";
			while(firstAddr < lastAddr){
				Dyninst::InstructionAPI::Instruction instr;
				instr = decoder.decode((unsigned char *)f->isrc()->getPtrToInstruction(firstAddr));
				cout <<std::hex << firstAddr <<": "<<instr.format() << "\n";
		...
		for (dp::Edge *ee : tues) {
            ...
            //if(eachpath[infomap[firstAddr].first].second < firstAddr){
                for(odata ww : infomap[lastAddr].second){
                    ...
                    if (!createAndInsertSnippet1(app, &exitPoint, ww)) {//before point
                        fprintf(stderr, "createAndInsertSnippet2 failed\n");
                        exit(1);
                    }
                    break;
